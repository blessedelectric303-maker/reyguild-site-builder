...
/* NEW PROPOSAL. The one thing somebody comes to this screen to do when the
   list has not got what they want in it. Full width along the bottom of a
   phone, where the thumb already is; top right on a desktop. */
.fl-newbig{
  position:fixed; left:12px; right:12px; bottom:12px; z-index:30;
  padding:16px 20px; border-radius:12px; cursor:pointer; font-family:inherit;
  font-size:16px; font-weight:800; letter-spacing:.02em; color:#16243F;
  background:var(--metal, linear-gradient(160deg,#F0CE7A,#CC9000 34%,#8A5E00 58%,#D89000 82%,#F0CE7A));
  border:1px solid rgba(255,255,255,.6);
  box-shadow:0 1px 0 rgba(255,255,255,.5) inset, 0 6px 18px rgba(0,0,0,.45);
}
.fl-newbig:hover{ filter:brightness(1.06); }
/* So the last row is never hidden behind it. */
.fl-list{ padding-bottom:84px; }

@media (min-width:900px){
  .fl-newbig{
    position:static; margin-left:auto; padding:10px 20px; font-size:14px;
    border-radius:9px; box-shadow:0 1px 0 rgba(255,255,255,.45) inset, 0 2px 6px rgba(0,0,0,.35);
  }
  .fl-list{ padding-bottom:0; }
}

/* A tab carrying news: the number of customers who just answered. */
.fl-pillcount{
  display:inline-block; min-width:17px; height:17px; margin-left:6px; padding:0 5px;
  border-radius:9px; background:#CC9000; color:#16243F;
